	...
	public PutGetFrame(int move) 
	{
		...
		search.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (mv==1)
				{
					String sql = "SELECT [dep] ,[global_rows] ,[rows] ,[columns] ,[id] "
						+ " FROM [medved].[dbo].[shelfs] WHERE [dep] = " + DepName.getDepName();
					shelfsTable.setModel(TableFill.tableModel(sql, columnNames));
				}
				else
				{
					String sql = "SELECT [dep] ,[global_rows] ,[rows] ,[columns] FROM [medved].[dbo].[storageView]"
